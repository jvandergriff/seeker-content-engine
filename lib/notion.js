...
export function mapWildlifeConnectRow(page) {
  const status = getProp(page, "Status", "select");
  const stage  = WC_STAGE[status] || "idea";
  return {
    id:        page.id,
    keyword:   getProp(page, "Keyword",       "title") || "(untitled)",
    status,
    ...
    kd:        getProp(page, "KD",            "number"),
    volume:    getProp(page, "Volume",        "number"),
    oppScore:  getProp(page, "Opp Score",     "number"),
    cluster:   getProp(page, "Cluster",       "select"),
    intent:    getProp(page, "Intent",        "select"),
    animal:    getProp(page, "Animal",        "text"),
    location:  getProp(page, "Location",      "text"),
